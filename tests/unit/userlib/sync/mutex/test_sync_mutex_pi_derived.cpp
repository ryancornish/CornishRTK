...
/* ============================================================================
 * A mutex acquired while its new owner is already READY is filed unboostable
 *
 * DEFECT DEMONSTRATION, expected to FAIL at 512b35c. See cross-core-defects.md
 * section 8.1. This test asserts the property the kernel does not currently
 * have, so a red result here is the finding reproducing, not a regression.
 *
 * The routing decision that makes a holder boostable is taken exactly once, in
 * set_thread_ready: a thread holding nothing goes into the ready matrix keyed on
 * base priority, a holder goes on the core's holder list, and only the holder
 * list is folded by the pick. The idempotency early return in that function means
 * a second admission never revisits the choice.
 *
 * Ownership, however, can arrive from a DIFFERENT core after that choice was
 * taken. hand_over files the resource into the new owner's slots itself, on the
 * releaser's core, and it must: ownership missing from held_slots is invisible
 * to the fold. So a thread that was admitted to the matrix while holding nothing
 * and then handed a mutex is a holder filed where no fold will ever look at it.
 *
 * Constructing that needs a group wait, because the mutex has to be armed at the
 * same time as something else that can wake the waiter first:
 *
 *   W, base 6 on core1, parks in wait_on_any(m, sem)
 *   N, base 3 on core1, a CPU-bound spinner, so W is READY but never RUNNING
 *   sem is released first  -> W is admitted to the ready matrix, holding nothing
 *   m is released second   -> hand_over gives m to W, which is still armed on it
 *   A, base 1 on core2, then blocks on m
 *
 * A's urgency must reach W, because A cannot proceed until W runs and releases
 * the mutex it now owns. W's core is busy with N at base 3, so the ONLY thing
 * that can get W the core is the pick folding its urgency to 1. W is in the
 * matrix at base 6, so the fold never considers it and N wins every pick. That is
 * an unbounded inversion: A waits on a thread that the scheduler will not run.
 *
 * Why the sem and not a second mutex. Both routes reach the same hole, but this
 * one is the harmful symptom. With two mutexes the missed update is the bridge
 * count, and the thread it fails to boost is the holder of a mutex the group
 * waiter no longer needs, so nobody's progress depends on it. Here the thread
 * that fails to be boosted is the one holding the resource A is blocked on.
 *
 * LIFETIME OF THIS TEST. mutex-first-class-plan.md bans mutexes from group waits
 * outright and deletes the timed mutex methods with them (D5), which makes the
 * composition below ill-formed. When that lands this test should stop compiling,
 * and that is the intended proof that the ban closed the hole. Replace it then
 * with a static assertion that the composition is rejected, and record the swap
 * here rather than deleting it silently.
 *
 * ONE-SIDED. The window needs W admitted to the matrix BEFORE m is handed over.
 * Both admissions coalesce on the same per-TCB request bit, so if they land in
 * one drain the routing is taken once, with m already held, and it is correct.
 * The driver separates them by waiting for N to make progress after the release,
 * which is evidence core1 took the poke and rescheduled. A round that misses the
 * window passes trivially: W simply runs early and returns the semaphore index.
 * So a PASS here is weak and a FAIL is strong, which is the right way round for
 * a defect demonstration.
 * ========================================================================= */
TEST_F(SyncMutexPiDerived_Test,
       GivenAMutexHandedToAnAlreadyReadyGroupWaiter_WhenAnUrgentThreadBlocksOnIt_ThenTheNewOwnerStillRuns)
{
   constexpr int reps = 20;

   // How far N must advance after the semaphore release before the mutex is
   // released. N only advances while it is running, so progress here means
   // core1 serviced the poke and went back to N, i.e. W is admitted.
   constexpr std::uint32_t settle_spins = 5000;
   ...
   for (int rep = 0; rep < reps; ++rep) {
      ...
      {
         mutex m;
         sync::semaphore sem{0};     // the non-mutex half of W's group wait
         sync::semaphore n_gate{0};  // holds N off until W has parked

         std::atomic<bool> m_taken{false};
         std::atomic<bool> w_parked{false};   // attested by the witness
         std::atomic<std::uint32_t> n_spins{0};
         std::atomic<bool> release_m{false};
         std::atomic<bool> m_released{false};
         std::atomic<bool> a_go{false};
         std::atomic<bool> a_got_m{false};
         std::atomic<bool> w_resumed{false};
         std::atomic<bool> stop_spinner{false};
         std::atomic<int>  failed_stage{0};
      };
      state s;

      // Owner: holds the mutex until told, so W has to park on it.
      thread owner(
         [&s]{
            s.m.lock();
            s.m_taken.store(true, std::memory_order_release);

            while (!s.release_m.load(std::memory_order_acquire)) {
               cyros_port_cpu_relax();
            }
            s.m.unlock();   // hands the mutex to W, which is armed and READY
            s.m_released.store(true, std::memory_order_release);
         },
         stacks[0].bytes, thread::priority(5), core0);

      // W: the group waiter. Parks on both, is woken by the semaphore, and is
      // then handed the mutex while it sits in the ready matrix.
      thread w(
         [&s]{
            while (!s.m_taken.load(std::memory_order_acquire)) {
               cyros_port_cpu_relax();
            }

            std::size_t const idx = this_thread::wait_on_any(s.m, s.sem);

            // Reaching here at all is the property under test: it means core1
            // preferred W over a base-3 spinner, which only a folded urgency of
            // 1 can justify.
            s.w_resumed.store(true, std::memory_order_release);
            s.stop_spinner.store(true, std::memory_order_release);

            // Index 0 means we own the mutex and A is waiting behind us. Index 1
            // means the window was missed and W woke on the semaphore instead,
            // in which case it never owned the mutex and there is nothing to
            // release.
            if (idx == 0) {
               s.m.unlock();
            }
         },
         stacks[1].bytes, thread::priority(6), core1);

      // Witness: lower than W on W's own core, so it runs only once W has parked.
      // This is what makes releasing N safe, N would otherwise starve W before it
      // ever reached the group wait.
      thread witness(
         [&s]{
            s.w_parked.store(true, std::memory_order_release);
         },
         stacks[2].bytes, thread::priority(7), core1);

      // N: better base priority than W and never blocks, so W is ready and not
      // running for as long as N spins. Publishes progress so the driver can tell
      // that core1 is servicing pokes between the two admissions.
      thread n(
         [&s]{
            s.n_gate.acquire();
            while (!s.stop_spinner.load(std::memory_order_acquire)) {
               s.n_spins.fetch_add(1, std::memory_order_release);
               cyros_port_cpu_relax();
            }
         },
         stacks[3].bytes, thread::priority(3), core1);

      // A: the urgent thread whose progress depends on W being run.
      thread a(
         [&s]{
            while (!s.a_go.load(std::memory_order_acquire)) {
               cyros_port_cpu_relax();
            }
            s.m.lock();     // donates urgency 1 to whoever owns m, which is W
            s.a_got_m.store(true, std::memory_order_release);
            s.m.unlock();
         },
         stacks[4].bytes, thread::priority(1), core2);

      // Driver: orders the two admissions, then observes. Every bail releases the
      // spinner, which lets core1 fall through to W on base priority alone and
      // unwinds the whole arrangement so the failure is an assertion, not a hang.
      thread driver(
         [&s]{
            if (!bounded_poll([&]{ return s.w_parked.load(std::memory_order_acquire); })) {
               s.failed_stage.store(1, std::memory_order_release);
               s.stop_spinner.store(true, std::memory_order_release);
               return;
            }

            // W is parked on both. Now take its core away.
            s.n_gate.release();
            if (!bounded_poll([&]{ return s.n_spins.load(std::memory_order_acquire) > 0; })) {
               s.failed_stage.store(2, std::memory_order_release);
               s.stop_spinner.store(true, std::memory_order_release);
               return;
            }

            // Admission one: W becomes READY holding nothing, so it is filed in
            // the ready matrix at base 6.
            auto const before = s.n_spins.load(std::memory_order_acquire);
            s.sem.release();

            // Let core1 actually service that before handing over the mutex.
            if (!bounded_poll([&]{
                   return s.n_spins.load(std::memory_order_acquire) > before + settle_spins;
                })) {
            ...
            // Admission two: the mutex is handed to a thread that is already
            // enqueued, so the routing is never revisited.
            s.release_m.store(true, std::memory_order_release);
            if (!bounded_poll([&]{ return s.m_released.load(std::memory_order_acquire); })) {
               s.failed_stage.store(4, std::memory_order_release);
               s.stop_spinner.store(true, std::memory_order_release);
               return;
            }

            // The urgent waiter arrives behind the mutex W now owns.
            s.a_go.store(true, std::memory_order_release);

            if (!bounded_poll([&]{ return s.a_got_m.load(std::memory_order_acquire); })) {
      ...
      EXPECT_NE(s.failed_stage.load(), 1) << "W never parked on the group wait";
      EXPECT_NE(s.failed_stage.load(), 2) << "the spinner never took core1";
      EXPECT_NE(s.failed_stage.load(), 3) << "core1 made no progress after the semaphore release";
      EXPECT_NE(s.failed_stage.load(), 4) << "the owner never released the mutex";
      EXPECT_NE(s.failed_stage.load(), 5)
         << "the urgent thread never acquired the mutex. Its owner W was handed "
            "the mutex while already enqueued in the ready matrix, so the pick "
            "never folds W's urgency and a base-3 spinner outranks it forever. "
            "W ran again: " << s.w_resumed.load();
